...
from bkchem import non_xml_writer
...
from bkchem.xml_writer import SVG_writer
...
## ##     hacksMenu.add( 'command', label=_('Clean'), command=self.clean)
    ...
    if plugins.__all__:
      for name in plugins.__all__:
        ...
        if plugin.name.endswith( "(Piddle)"):
          from bkchem.plugins import tk2piddle
  ...
  def init_mode_buttons( self):
    ...
    self.radioFrame = Frame( self.main_frame)
    self.radioFrame.grid( row=1, sticky='we')
    self.radiobuttons = Pmw.RadioSelect(self.radioFrame,
                                        buttontype = 'button',
                                        selectmode = 'single',
                                        orient = 'horizontal',
